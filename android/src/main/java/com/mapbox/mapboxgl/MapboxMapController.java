    ...
    public void onMethodCall(MethodCall call, MethodChannel.Result result) {
        ...
        switch (call.method) {
            ...
            case "map#queryRenderedFeatures": {
                ...
                List<String> matchLayerIds = new ArrayList<>();
                for (String layerId : layerIds) {
                    if (style != null) {
                        List<Layer> layerList = style.getLayers();
                        for (Layer layerTemp : layerList) {
                            if (layerTemp.getId().startsWith(layerId)) {
                                matchLayerIds.add(layerTemp.getId());
                            }
                        }
                    }
                }
                ...
                if (call.hasArgument("x")) {
                    ...
                    features = mapboxMap.queryRenderedFeatures(pixel, filterExpression, matchLayerIds.toArray(new String[0]))
                    ;
                } else {
                    ...
                    features = mapboxMap.queryRenderedFeatures(rectF, filterExpression, matchLayerIds.toArray(new String[0]));
                }
