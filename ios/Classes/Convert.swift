...
class Convert {
    class func interpretMapboxMapOptions(options: Any?, delegate: MapboxMapOptionsSink) {
        ...
        if let enableLogo = options["enableLogo"] as? Bool {
            delegate.setEnableLogo(enableLogo: enableLogo)
        }
        if let enableAttribution = options["enableAttribution"] as? Bool {
            delegate.setEnableAttribution(enableAttribution: enableAttribution)
        }
        ...
        if let languageEnable = options["languageEnable"] as? Bool {
            delegate.setLanguageEnable(languageEnable: languageEnable)
        }
    ...
    class func parseCameraUpdate(cameraUpdate: [Any], mapView: MGLMapView) -> MGLMapCamera? {
        ...
        case "newLatLngBounds":
            ...
            if cameraUpdate.count >= 6 {
                guard let left = cameraUpdate[2] as? CGFloat,
                let top = cameraUpdate[3] as? CGFloat,
                let right = cameraUpdate[4] as? CGFloat,
                let bottom = cameraUpdate[5] as? CGFloat
                else { return nil }
                return mapView.cameraThatFitsCoordinateBounds(MGLCoordinateBounds.fromArray(bounds), edgePadding: UIEdgeInsets.init(top: top, left: left, bottom: bottom, right: right))
            } else {
